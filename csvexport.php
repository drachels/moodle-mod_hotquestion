...
function array_to_csv_download($array, $filename = "export.csv", $delimiter=";") {
    ...
    $headings = array(get_string('firstname', 'moodle'),
                      get_string('lastname', 'moodle'),
                      get_string('id', 'hotquestion'),
                      get_string('hotquestion', 'hotquestion'),
                      get_string('content', 'hotquestion'),
                      get_string('userid', 'moodle'),
                      get_string('time', 'hotquestion'),
                      get_string('anonymous', 'moodle'));
    fputcsv($f, $headings, $delimiter);
    foreach ($array as $hq) {
		$fields = array($hq->firstname.' '.$hq->lastname, $hq->id, $hq->hotquestion, $hq->content, $hq->userid, date('d. M Y G:i', $hq->time), $hq->anonymous);
...
$hq_id = optional_param('hotquestion', 0, PARAM_INT);
...
array_to_csv_download($grds, get_string('exportfilename', 'hotquestion'));
